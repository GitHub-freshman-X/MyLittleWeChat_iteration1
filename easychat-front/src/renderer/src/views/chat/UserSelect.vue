<template>
  <div>
    <Dialog v-model:show="dialogConfig.show" :title="dialogConfig.title" :buttons="dialogConfig.buttons" width="660px"
      ...
      <el-transfer v-model="formData.selectContacts" :titles="['全部', '已选']" :format="{
        ...
        <template #default="{ option }">
          <div class="select-item">
            <div class="avatar">
              <AvatarBase :userId="option.contactId" :width="30" :borderRadius="5" :showDetail="false"></AvatarBase>
            </div>
...
const show = ({ contactList, groupId, opType }) => {
  dialogConfig.value.title = opType == 0 ? '移除群成员' : '添加群成员'
